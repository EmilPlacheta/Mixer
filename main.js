...
const displayCocktails = function() {
  ...
  let result = cocktails.filter(recipe =>
    recipe.spirit.every(spirit => availabeSpirits.includes(spirit))
  );
  ...
  if (availabeSpirits === '') {
    ...
    resultsArr.push(
      newPara.append(
        document.createTextNode(`Something got mixed up, try again?`)
      )
    );
  } else {
    for (let i = 0; i < result.length; i++) {
      resultsArr.push(
        newPara.append(document.createTextNode(`${result[i].name},  `))
      );
    }
